...
@Component
public class PagamentoCartaoCreditoImpl  implements PagamentoStrategy {
    @Autowired
    ...
    public void pagar(Pagamento pagamento) {
        try {
            ...
            logger.error("Pagamento por Cartão de Credito salvo com sucesso :)");
        }catch (Exception e){
            logger.error("Erro ao processar pagamento por Cartão de Credito. Erro: {}", e.getMessage());
            throw e;
        }
    }
    ...
    public OperacaoEnum getOperacao() {
        return OperacaoEnum.CARTAO_CREDITO;
    }
}
